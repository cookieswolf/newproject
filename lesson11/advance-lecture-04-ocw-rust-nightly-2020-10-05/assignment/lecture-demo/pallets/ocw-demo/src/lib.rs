...
pub const HTTP_HEADER_USER_AGENT: &str = "Tester";
...
struct PolkadotInfoData {
	#[serde(deserialize_with = "de_string_to_bytes")]
	priceUsd: Vec<u8>,
}
#[derive(Deserialize, Encode, Decode, Default)]
struct PolkadotInfo {
	data:PolkadotInfoData,
        ...
        D: Deserializer<'de>,
{
    ...
    let i = s.find('.').unwrap(); // 小数点的索引
    let result = &s[0..i + 4]; // 三位小数
...
decl_storage! {
	trait Store for Module<T: Trait> as OcwDemo {
		/// A vector of recently submitted numbers. Bounded by NUM_VEC_LEN
		Numbers get(fn numbers): Vec<Vec<u8>>;
	}
}
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		#[weight = 10000]
		pub fn submit_number_signed(origin, number: Vec<u8>) -> DispatchResult {
			let who = ensure_signed(origin)?;
			debug::info!("submit_number_signed: ({:?}, {:?})", number.clone(), who);
			Self::append_or_replace_number(number.clone());

			Self::deposit_event(RawEvent::NewNumber(Some(who), number.clone()));
			Ok(())
		}

		#[weight = 10000]
		pub fn submit_number_unsigned(origin, number: Vec<u8>) -> DispatchResult {
			let _ = ensure_none(origin)?;
			debug::info!("submit_number_unsigned: {:?}", number.clone());
			Self::append_or_replace_number(number.clone());

			Self::deposit_event(RawEvent::NewNumber(None, number.clone()));
			Ok(())
		}
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			let result = match modu {
				0 => Self::fetch_polkadot_info(0),
				1 => Self::fetch_polkadot_info(1),
				2 => Self::fetch_polkadot_info(2),
				_ => Self::fetch_polkadot_info(0),
			};
...
impl<T: Trait> Module<T> {
	...
	///   the bounded length.
	fn append_or_replace_number(number: Vec<u8>) {
	...
	fn fetch_n_parse() -> Result<PolkadotInfo, Error<T>> {
	...
	fn offchain_signed_tx(number: Vec<u8>) -> Result<(), Error<T>> {
		...
		let result = signer.send_signed_transaction(|_acct|
			...
			Call::submit_number_signed(number.clone())
		);
	...
	fn offchain_unsigned_tx(number: Vec<u8>) -> Result<(), Error<T>> {
		//let number: u32 = block_number.try_into().unwrap_or(0) as u32;
	...
	fn offchain_unsigned_tx_signed_payload(number: Vec<u8>) -> Result<(), Error<T>> {
		...
		if let Some((_, res)) = signer.send_unsigned_transaction(
			|acct| Payload {number:number.clone(), public: acct.public.clone() },
